...
if PLANET_FULL_NAMES_LIST and PLANET_SHORT_NAMES_LIST and \
    ...
    print(
        "WARNING: Could not reliably find PyJHora's planet name lists (e.g., PLANET_NAMES, PLANET_SHORT_NAMES via const or utils). Universal name expansion might be limited.")
...
def _format_datetime_from_jd(jd_val: float) -> str:
    ...
    # ---- Optional: Keep this debug line for one more run if you want to be extra sure ----
    # print(f"DEBUG (Inside _format_datetime_from_jd): jd_to_gregorian({jd_val}) returned: {greg_date_tuple}, type: {type(greg_date_tuple)}, len: {len(greg_date_tuple) if isinstance(greg_date_tuple, (list, tuple)) else 'N/A'}")
    # ---- End Optional Debug ----

    if not isinstance(greg_date_tuple, (list, tuple)) or len(greg_date_tuple) < 4:
        print(f"ERROR: jd_to_gregorian returned an unexpected value: {greg_date_tuple}")
        # Return a placeholder or raise a more specific error
        return "0000-00-00 00:00:00"

    year = int(greg_date_tuple[0])
    month = int(greg_date_tuple[1])
    day = int(greg_date_tuple[2])
    ...
    # Handle potential carry-over if seconds round up to 60, then minutes to 60
    if second >= 60:
    ...
    if minute >= 60:
        ...
        hour += 1
    # Note: hour carry-over to day is generally not expected here if jd_to_gregorian gives correct day
    if hour >= 24:
        # This case might indicate a deeper issue or an edge case in jd_to_gregorian
        # For now, let's cap it, but it's worth noting if it happens.
        # print(f"Warning: Hour calculated as {hour}, capping to 23.")
        hour = hour % 24  # Or handle day increment if necessary
...
def extract_kn_rao_chara_dasha_detailed(
    ...
    for cycle_num in range(max_md_cycles):
        if elapsed_total_years_for_mds >= total_years_to_calculate_md_ad:
            break
        for md_rasi_index_current_cycle in md_progression_rasis:
            if elapsed_total_years_for_mds >= total_years_to_calculate_md_ad:
                break
            md_start_jd_current_md = current_jd_for_mds
            md_total_duration_years_full = jhora_chara_dhasa._dhasa_duration_knrao_method(
                d1_chart_positions, md_rasi_index_current_cycle
            )
            ...
            if actual_md_run_duration_this_iteration <= 0:
                continue
            ...
            try:
                ...
                continue
            for i in range(12):
                if current_ad_start_jd >= md_end_jd_current_md:
                    break
                ad_rasi_index = md_progression_rasis[(ad_start_progression_index + i) % 12]
                ad_start_jd_this_ad = current_ad_start_jd
                ad_end_jd_this_ad = min(
                    ad_start_jd_this_ad + (ad_proportional_duration_years * one_year_in_days),
                ...
                all_periods_md_ad.append({
                    ...
                    "start_date": f"{int(ad_start_tuple[0]):04d}-{int(ad_start_tuple[1]):02d}-{int(ad_start_tuple[2]):02d}",
                    "end_date": f"{int(ad_end_tuple[0]):04d}-{int(ad_end_tuple[1]):02d}-{int(ad_end_tuple[2]):02d}",
                })
                ad_start_date_obj = datetime.date(int(ad_start_tuple[0]), int(ad_start_tuple[1]),
                                                  int(ad_start_tuple[2]))
                ad_end_date_obj = datetime.date(int(ad_end_tuple[0]), int(ad_end_tuple[1]), int(ad_end_tuple[2]))
                if max(ad_start_date_obj, filter_start_date_obj) < min(ad_end_date_obj, filter_end_date_obj):
                    ...
                    for j in range(12):
                        ...
                        pd_start_date_obj = datetime.date(int(pd_start_tuple[0]), int(pd_start_tuple[1]),
                                                          int(pd_start_tuple[2]))
                        pd_end_date_obj = datetime.date(int(pd_end_tuple[0]), int(pd_end_tuple[1]),
                                                        int(pd_end_tuple[2]))
                        if max(pd_start_date_obj, filter_start_date_obj) < min(pd_end_date_obj, filter_end_date_obj):
                            ...
                            for k in range(12):
                                ...
                                sd_start_date_obj = datetime.date(int(sd_start_tuple[0]), int(sd_start_tuple[1]),
                                                                  int(sd_start_tuple[2]))
                                sd_end_date_obj = datetime.date(int(sd_end_tuple[0]), int(sd_end_tuple[1]),
                                                                int(sd_end_tuple[2]))
                                if max(sd_start_date_obj, filter_start_date_obj) < min(sd_end_date_obj,
                                    ...
                                    all_periods_filtered_to_sd_level.append({
                                        ...
                                        "start_date": f"{int(sd_start_tuple[0]):04d}-{int(sd_start_tuple[1]):02d}-{int(sd_start_tuple[2]):02d}",
                                        "end_date": f"{int(sd_end_tuple[0]):04d}-{int(sd_end_tuple[1]):02d}-{int(sd_end_tuple[2]):02d}",
                                    })
...
# --- Vimsottari Dasha Text Generation ---
def generate_vimsottari_text_for_universal_parser(h_obj: Horoscope) -> str:
    ...
    mahadashas_dict = jhora_vimsottari_std.vimsottari_mahadasa(
        jd=jd_dob, place=h_obj.Place, dhasa_starting_planet=1
    )
    md_summary_lines = []
    md_detailed_blocks = []
...
def parse_dasha_text_content(text_content: str, person_name: str, dasha_system_source_id: str) -> dict:
    ...
    period_line_regex = re.compile(
        r"^(?P<indent>\s*)(?P<name>[A-Za-z0-9\s]+?)\s*(?P<type>MD|AD|PD|SD)?:\s*"
        r"(?P<start_date>\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2})\s*-\s*"
        r"(?P<end_date>\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2})"
    )
    ...
    sub_period_section_headers = {
        "Antardasas in this MD:": ("Antardasha", 2),
    ...
    for line_num, line_content in enumerate(lines):
        ...
        if stripped_line == "Maha Dasas:":
            is_in_summary_block = True
            continue
        if stripped_line in sub_period_section_headers:
            is_in_summary_block = False
            continue
        ...
        if match:
            ...
            if period_type_from_line == "MD":
                level = 1;
                period_type_str = "Mahadasha"
            elif period_type_from_line == "AD":
                level = 2;
                period_type_str = "Antardasha"
            elif period_type_from_line == "PD":
                level = 3;
                period_type_str = "Pratyantardasha"
            elif period_type_from_line == "SD":
                level = 4;
                period_type_str = "Sookshma-antardasha"
            else:
                continue
            period_obj = {
                "level": level, "period_type": period_type_str,
                "name": _expand_name_universal(name_short, current_dasha_system_name),
                "start_datetime": _parse_datetime_universal(data['start_date']),
                "end_datetime": _parse_datetime_universal(data['end_date']),
                "sub_periods": []
            }
            ...
            if not context_stack:
                ...
                if level == parent_period['level'] + 1:
...
def main():
    ...
    try:
        ...
        h = Horoscope(
            date_in=drik_birth_date_obj, birth_time=current_birth_data.tob_str,
    ...
    try:
        d1_planet_positions = jhora_charts.rasi_chart(
            jd_at_dob=h.julian_day, place_as_tuple=h.Place, ayanamsa_mode=h.ayanamsa_mode
        )
    except Exception as e:
    ...
    if h and d1_planet_positions:
        try:
            ...
            print(f"ERROR calculating/saving K.N. Rao Chara Dasha (flat): {e}");
            traceback.print_exc()
    if h:
        print("\nGenerating Vimsottari Dasha text for universal parser...")
        vimsottari_text_generated = generate_vimsottari_text_for_universal_parser(h)
        debug_text_filepath = full_output_path / f"{current_birth_data.name.replace(' ', '_')}-Vimsottari_GeneratedText.txt"
        try:
            with open(debug_text_filepath, 'w', encoding='utf-8') as f:
                f.write(vimsottari_text_generated)
            print(f"Saved generated Vimsottari text for debugging to: {debug_text_filepath}")
        except Exception as e:
        ...
        if "Error during generation." not in vimsottari_text_generated:
            ...
            dasha_system_source_id = "Vimsottari Dasa (Generated from PyJHora)"
            nested_vimsottari_json = parse_dasha_text_content(
                vimsottari_text_generated, current_birth_data.name, dasha_system_source_id
            )
            json_filename_vimsottari = f"{current_birth_data.name.replace(' ', '_')}-VimsottariDasa_Nested.json"
            filepath_vimsottari_nested = full_output_path / json_filename_vimsottari
            try:
                with open(filepath_vimsottari_nested, 'w', encoding='utf-8') as f:
                    json.dump(nested_vimsottari_json, f, indent=2, ensure_ascii=False)
                print(f"Successfully saved Vimsottari Dasha (Nested JSON) to: {filepath_vimsottari_nested}")
            except Exception as e:
                print(f"ERROR saving Vimsottari Dasha Nested JSON: {e}");
                traceback.print_exc()
        else:
            print("Skipping Vimsottari Dasha parsing due to generation error.")
    print("\n--- Processing Complete ---")
